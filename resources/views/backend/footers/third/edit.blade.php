            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        ...
                        <a class="btn btn-sm btn-success" href="{{ route('admin.third.index') }}">Back</a>
                    </div>
                    <div class="card-body card-block">
                        <form action="{{ route('admin.third.update', $footer->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PATCH')
                            ...
                            <div class="row  form-group service_link">
...
<script type="text/javascript">
    (function ($) {
    "use strict";
    $('.service_link').not(":first").find('.col-md-9').append('<button type="button" class=" delete_item btn btn-danger btn-sm mt-2">Delete</button>');
    $(document).on('click', '#add_item', function() {
        $('.service_link:first').clone().insertAfter(".service_link:last").find('.col-md-9').append('<button type="button" class=" delete_item btn btn-danger btn-sm mt-2">Delete</button>');
    }).on('click', '.delete_item', function() {
        $(this).closest('.service_link').remove();
    });
