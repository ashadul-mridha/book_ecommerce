            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    ...
                    <div class="card-header d-flex justify-content-between">
                        <strong class="card-title">Foote First Cloumn Data</strong>
                        @can('manage-footers')
                        @if (count($footers) <= 0) <a class="btn btn-sm btn-success"
                            href="{{ route('admin.first.create') }}">Add
                            Fist Cloumn</a>
                            @endif
                            @endcan
                    </div>
                    <div class="card-body">
                        <table id="bootstrap-data-table" class="table table-striped table-bordered">
                            ...
                            <tbody>
                                ...
                                <tr>
                                <td>{{ $id }}</td>
                                    <td><img src="{{asset('images/footer/'.$footer->logo)}}" alt="Logo"
                                            width="100px">
                                    </td>
                                    <td width="15%">{{ $footer->description }}</td>
                                    <td>
                                        @php
                                            $link = explode('|', $footer->social_icon);
                                    ...
                                    <td>
                                        ...
                                        <a class="btn btn-sm btn-info"
                                            href="{{ route('admin.first.edit', $footer->id) }}">
                                            Edit
                                        </a>
                                        @if (count($footers) > 1)
                                        <form action="{{ route('admin.first.destroy', $footer->id) }}" method="POST"
                                            class="d-inline">
