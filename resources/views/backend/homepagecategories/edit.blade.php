            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header d-flex justify-content-between">
                        <strong class="card-title">Update Home Page Category</strong>
                        <a class="btn btn-sm btn-success" href="{{ route('admin.homepagecategory.index') }}">Back</a>
                    </div>
                    <div class="card-body card-block">
                        <form action="{{ route('admin.homepagecategory.update', $homepagecat->id) }}" method="POST"
                                    ...
                                    <div class=" form-group">
                                        <select name="category_id" id="category_id"
                                            ...
                                            <option value="{{$category->id}}"
                                                {{ ( old('category_id') || ($category->id == $homepagecat->category_id)) ? 'selected' : ''}}>
                                        ...
                                        <div class="checkbox">
                                            <label for="status" class="form-check-label ">
                                                <input type="checkbox" id="status" name="status" value="on"
                                                    ...
                                                    {{ $homepagecat->status == 1 ? 'checked':''}}>Publish
                                            </label>
                                        </div>
                                    </div>
                                </div>
